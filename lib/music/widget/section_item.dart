...
import 'package:flutter_application_1/music/mixins/enhanced_image.dart';
...
class SectionItem extends StatefulWidget {
  const SectionItem({required this.section, this.isMore = false, super.key, required this.ytMusic});
  final Map section;
  final bool isMore;
...
class _SectionItemState extends State<SectionItem> {
  ...
  loadMoreItems(){
    if(widget.section['continuation']!=null){
      ...
      widget.ytMusic.getMoreItems(continuation: widget.section['continuation']).then((value) {
        setState(() {
    ...
    return widget.section['contents'].isEmpty
        ...
        : Column(
            children: [
              ...
              if(widget.section['continuation']!=null && !loadingMore) OutlinedButton(onPressed: loadMoreItems, child:const Text("Load More"))
            ],
          );
  }
}
      ...
      return SizedBox(
        ...
        child: ListView.separated(
          ...
          borderRadius:
              ...
              color: Platform.isAndroid ? Colors.transparent : null,
